...
| TST-002 | Run initial recommendations | Three ordered Recommendations render with scores, reasons, citations, confidence, version, and data mode; connected evidence shows Coordinator → Scout → Analyst → Coach → Coordinator messages in one Band room | Pending |
| TST-003 | Disable all external credentials | Full local journey through feedback and reranking succeeds; Actian and Band say `demo_fallback`, and every other integration says `demo_fallback` or `disabled` | Pending |
